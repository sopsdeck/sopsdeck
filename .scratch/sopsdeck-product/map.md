...
**Status: destination reached.** Remaining work is vertical build tickets (TDD), not this map.

## Notes

- Domain: local-first encrypted configuration management. Use [the domain glossary](../../CONTEXT.md).
- Build with `/tdd` one tracer-bullet phase at a time from [Lock public test seams and delivery phases](issues/15-lock-public-test-seams-and-delivery-phases.md). Consult `/wayfinder` and `/domain-modeling` if a build slice invents policy — it must not.
...
- [Lock public test seams and delivery phases](issues/15-lock-public-test-seams-and-delivery-phases.md) — CLI/Git/GitHub/scan/MCP/write seams; eight tracer-bullet phases to a signed macOS release.

## Not yet specified

- Screen-level onboarding/empty/error/keyboard chrome on top of the locked IA.
...
- Production implementation is outside this decision map; it is tracked as vertical build tickets from phase 1 of [Lock public test seams and delivery phases](issues/15-lock-public-test-seams-and-delivery-phases.md).
