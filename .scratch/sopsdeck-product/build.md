...
1. Incomplete tracer-bullet phases in issue 15 (Review/History/Restore, `.sopsdeck.toml` Publish mappings, scan, MCP, signed macOS release). Request PR / re-encrypt PR still open on recipients.
...
| 3 | Git Commit / Sync | in progress | commit `-m` `-f`; Sync = fetch + ff-only pull + push; refuse dirty/diverge; Review semantic diff | Secret History; Restore; three-way (07) |
