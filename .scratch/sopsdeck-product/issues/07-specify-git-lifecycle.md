...
`sopsdeck commit -m … -f` and `sopsdeck sync` (fetch, `pull --ff-only`, push) are tested, including diverge refusal and dirty-worktree stop. `sopsdeck review -f` prints a plaintext semantic diff of uncommitted keys vs HEAD, or a secret-aware three-way (base / ours / theirs) when the file is unmerged and all three stages decrypt. Otherwise it leaves the conflict to Git. `sopsdeck history -f` is `git log` on the file; `get --at REV` decrypts that revision. `sopsdeck restore -f FILE --at REV` copies those values into the current file and does not commit. Desktop Commit/Sync/Review/History/Restore invoke those commands. Commit message is user-supplied; prefill is [21](21-desktop-chrome-polish.md).
