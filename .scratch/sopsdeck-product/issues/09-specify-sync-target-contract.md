...
`sopsdeck publish -f FILE [--prefix] [--yes] [--prune]` talks to `SOPSDECK_GITHUB_API` (studio uses `internal/githubfake`). Dry-run unless `--yes`. `--mapping` prints the resolved repo, environment, prefix, and keys without calling GitHub. Drive invoke `publish_managed_file` and `get_publish_mapping`. Inspector Publish shows mapping, prefix, and opt-in prune. `.sopsdeck.toml` maps a Managed File to `repo`, optional `environment`, `prefix`, `keys`, and last-published `published` names; CLI `--prefix` overrides the mapping; prune deletes only previously published prefixed names. Environment mappings `PUT` `/repos/{owner}/{repo}/environments/{environment}/secrets/{name}`. Auth is `GH_TOKEN`, `GITHUB_TOKEN`, or `gh auth token` (Bearer). Publish fetches the target public key and sends Libsodium-compatible sealed boxes; the local fake returns a valid public key without exposing plaintext values.
