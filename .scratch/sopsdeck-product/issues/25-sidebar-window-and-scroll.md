...
- [ ] From the sidebar, the user can add a new Managed File; after save it appears in the tree and opens in the editor.
- [ ] Theme toggle is an icon (or equally compact control), still persists across reload.
- [ ] Packaged/dev app title and Dock/taskbar name are **Sopsdeck** with the product icon, not “desktop”.
- [ ] Outer window does not rubber-band-scroll to empty body chrome; lists/editor/inspector scroll inside panes. Scrollbars match brand (minimal, not OS-default ugly if the WebView allows).
...
Captured 2026-08-28 from human-found review. Kind: bug + idea. Spec: [05](05-define-project-manifest-and-registration.md), [04](04-validate-folder-first-workspace.md).
