...
| `./scripts/cli-video.mjs` | Regenerates the Castkit CLI MP4 and its public site copy                                         |
| `./scripts/scan`          | `govulncheck` and `bun audit` (not in `./scripts/check`)                                         |
| `./scripts/dev`           | Builds `./sopsdeck` and launches the browser app with that binary (`--team` for two local Users) |
